...
export default function Billing() {
  ...
  const handleCancel = async () => {
    if (!window.confirm("Cancel your subscription? You'll keep access until your current period ends.")) {
      return;
    }
    setCancelling(true);
    try {
  ...
  if (error) return <div className="billing-page">{error}</div>;
  if (!status) return null;

  const sub = status.subscription;
  const endsAt = sub?.endsAt || sub?.renewsAt;
  const daysLeft = endsAt
    ? Math.max(0, Math.ceil((new Date(endsAt) - new Date()) / 86400000))
    : null;

  return (
    <div className="billing-page" style={{ maxWidth: 480, margin: "0 auto", padding: 24 }}>
      ...
      {status.plan === "free" && (
        <p>You're currently on the Free plan. Upgrade anytime from Pricing.</p>
      )}
    </div>
  );
}